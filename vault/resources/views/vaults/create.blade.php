...
<div style="max-width: 560px;" class="mx-auto">
    ...
    <form method="POST" action="{{ route('vaults.store') }}" class="card shadow-sm">
        @csrf
        <div class="card-body">
            <div class="mb-3">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="icon" id="icon" class="form-control" value="{{ old('icon') }}" maxlength="50" placeholder="e.g. emoji">
                </div>
                <div class="col-md-6 mb-3">
                    <label for="color" class="form-label small fw-semibold">Color</label>
                    <input type="color" name="color" id="color" class="form-control form-control-color w-100" value="{{ old('color', '#3366cc') }}">
                </div>
            </div>
        </div>
        <div class="card-footer bg-white d-flex gap-2 justify-content-end">
            <a href="{{ route('vaults.index') }}" class="btn btn-outline-secondary">Cancel</a>
            <button type="submit" class="btn btn-primary"><i class="bi bi-plus-lg me-1"></i>Create Vault</button>
        </div>
    </form>
</div>
@endsection
